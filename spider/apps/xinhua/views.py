        ...
        if news_url.startswith('h'):
            ...
            if response.status_code == 200:
                ...
                for news_img_item in news_img_xpa:
                    news_img = news_img + setting.url + news_img_item.split('/')[-1] + '\r\n'
...
def get_news_list_2(url_index, url_item):
    """获取政务里面的news_url"""
    response = requests.get(url_item, headers=setting.headers)
    news_info_HTML = response.text
    news_info_etr = etree.HTML(news_info_HTML)
...
def person_new_news():
    """人物最新新闻"""
    person_new_news_url = setting.person_new_news_url
...
@loggerInFile("xinhua_log.txt", run_file)
def run():
    news_main()
    # person_new_news()
